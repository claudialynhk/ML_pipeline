...
textRDD = sc.textFile("/databricks-datasets/power-plant/data")  
...
plantDF = sqlContext.read.format("com.databricks.spark.csv").option("delimiter", "\t").option("header", "true").option("inferSchema", "true").load("/databricks-datasets/power-plant/data") 
...
altPlantDF = sqlContext.read.format("com.databricks.spark.csv").option("delimiter", "\t").option("header", "true").load("/databricks-datasets/power-plant/data", schema = customSchema) 
...
# MAGIC select V as VacuumSpeed , PE as PowerOutput from plant_dataset
...
# MAGIC -- TODO: Replace <FILL_IN> with the appropriate SQL command.
# MAGIC 
# MAGIC select AP as AtmosphericPressure , PE as PowerOutput from plant_dataset
...
(split15DF, split85DF) = datasetDF.randomSplit([0.15,0.85], 1900009193L)
...
# TEST
Test.assertEquals(round(rmseDT, 2), 5.14, "Incorrect value for rmseDT")
...
rf.setPredictionCol("Prediction_PE").setLabelCol("PE").setFeaturesCol("features").setSeed(190088121L).setMaxDepth(8).setNumTrees(25)
...
crossval.setNumFolds(3).setEstimator(rfPipeline).setEvaluator(regEval)
